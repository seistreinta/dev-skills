...
- **`/ai-usage-report`** measures your own work rather than moving it forward: what share of your commits were AI-assisted. It reads the local Claude Code transcripts and git history on your machine, uploads nothing, and reads no prompt or response text. The number is an **upper bound**, since a commit counts once Claude edited any of its changed files in the previous 21 days, and anything written outside Claude Code reads as zero. Off every flow: reach for it when the question is how much, not what next.
